...
best_settings_df.to_csv('../concat/results/' + pretrained_model_name_or_path.replace('/', '_') + '_hyperparams.csv', index = False)
...
model_checkpoint = '../concat/models/' + pretrained_model_name_or_path.replace('/', '_')
final_trainer.train()
...
out.to_csv('../concat/results/' + pretrained_model_name_or_path.replace('/', '_') + '_preds.csv', index = False)
